...
| containers | cask `docker-desktop` (engine + CLI) | dnf `podman` — rootless by default; add `podman-docker` if you want the `docker` command name || jetbrains-toolbox · gitkraken · obsidian · keymapp · google-chrome                                                                                     | cask                                         | **hand** — flatpak / official tarball / vendor repo     |
