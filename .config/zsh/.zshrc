...
# ------------------ GPG ------------------
# Only when stdin really is a terminal: elsewhere `tty` prints "not a tty" on
# STDOUT, GPG_TTY ends up holding that string and every pinentry fails.
if [[ -t 0 ]]; then
  GPG_TTY=$(tty)
  export GPG_TTY
fi
...
# menu no is REQUIRED by fzf-tab: it must capture the unambiguous prefix
# instead of zsh opening its own menu.
...
# Only if zinit really loaded (zinit.zsh gives up without git or network):
# otherwise every TAB on `zinit` autoloads a completion for a missing command.
...
# directory, and a failed symlinks step must not break every zsh start.
...
# ------------------ Tools init ------------------
# Guarded: a missing tool must never break the shell. The mise completion is
# generated by the `usage` CLI, itself installed by ~/.config/mise/config.toml.
if command -v mise >/dev/null 2>&1; then
  # --shims, not the hook: same resolution as scripts get, and no per-project
  # [env] is used here.
  eval "$(mise activate zsh --shims)"
  # Cached once: regenerating at every startup costs a fork for an identical
...
# AFTER `bindkey -v`: fzf rebinds Ctrl-R/Ctrl-T/Alt-C and the bindings must
# land in the vi keymaps.
